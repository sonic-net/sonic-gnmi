...
func Acknowledge(conn *grpc.ClientConn, ctx context.Context) {
	fmt.Println("Healthz Acknowledge")
	...
	/*
		if err != nil {
			if status.Code(err) == codes.Unimplemented {
				fmt.Println("Healthz.Acknowledge RPC is not implemented on the target device.")
				return
			}
			panic(fmt.Sprintf("Healthz.Acknowledge RPC failed: %v", err))
		}
	...
	*/
	if err != nil {
		fmt.Sprintf("Healthz.Acknowledge RPC failed: %v", err)
	}
	fmt.Printf("Acknowledge response: %+v\n", resp.Status)
}

func Artifact(conn *grpc.ClientConn, ctx context.Context, id string) {
	fmt.Println("Healthz Artifact client")
	ctx = utils.SetUserCreds(ctx)
	client := pb.NewHealthzClient(conn)
	...
	if err != nil {
		fmt.Errorf("failed to call Artifact RPC: %v", err)
	}
	...
	for {
		...
		if err != nil {
			fmt.Errorf("error receiving artifact resp: %v", err)
		}
		...
		case *pb.ArtifactResponse_Bytes:
			fmt.Printf("Received bytes chunk: %d bytes\n", len(c.Bytes))
			// You can append to a file or buffer here
