...
type Service interface {
	...
	Download(url string, save_as string) error
	Install(where string) error
}
...
func (c *DbusClient) Download(url string, save_as string) error {
	common_utils.IncCounter(common_utils.DBUS_DOWNLOAD)
	modName := "image_service"
	busName := c.busNamePrefix + modName
	busPath := c.busPathPrefix + modName
	intName := c.intNamePrefix + modName + ".download"
	_, err := DbusApi(busName, busPath, intName, 800, url, save_as)
	return err
}

func (c *DbusClient) Install(where string) error {
	common_utils.IncCounter(common_utils.DBUS_DOWNLOAD)
	modName := "image_service"
	busName := c.busNamePrefix + modName
	busPath := c.busPathPrefix + modName
	intName := c.intNamePrefix + modName + ".install"
	_, err := DbusApi(busName, busPath, intName, 800, where)
	return err
}
