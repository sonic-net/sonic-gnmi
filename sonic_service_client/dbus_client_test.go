...
func TestOSActivateSuccess(t *testing.T) {
	req := `{"image":"SONiC.master.0"}`
	expected := "Activated"
	...
	mock2 := gomonkey.ApplyMethod(reflect.TypeOf(&dbus.Object{}), "Go",
		func(obj *dbus.Object, method string, flags dbus.Flags, ch chan *dbus.Call, args ...interface{}) *dbus.Call {
			if method != "org.SONiC.HostService.gnoi_os_mgmt.activate" {
				t.Errorf("Wrong method: %v", method)
			}
			ret := &dbus.Call{
				Err: nil,
	...
	client, _ := NewDbusClient()
	result, err := client.OSActivate(req)
	if err != nil {
		t.Errorf("OSActivate should succeed: %v", err)
	}
	if result != expected {
		t.Errorf("Expected '%s', got '%s'", expected, result)
	}
...
func TestOSActivateFail(t *testing.T) {
	req := `{"image":"SONiC.master.0"}`
	...
	client, _ := NewDbusClient()
	_, err := client.OSActivate(req)
	if err == nil || err.Error() != errMsg {
		t.Errorf("Expected error '%s', got '%v'", errMsg, err)
	}
}

func TestOSVerifySuccess(t *testing.T) {
	req := `{"image":"SONiC.master.0"}`
	expected := "Verified"
	...
	mock2 := gomonkey.ApplyMethod(reflect.TypeOf(&dbus.Object{}), "Go",
		func(obj *dbus.Object, method string, flags dbus.Flags, ch chan *dbus.Call, args ...interface{}) *dbus.Call {
			if method != "org.SONiC.HostService.gnoi_os_mgmt.verify" {
				t.Errorf("Wrong method: %v", method)
			}
			ret := &dbus.Call{
				Err: nil,
	...
	client, _ := NewDbusClient()
	result, err := client.OSVerify(req)
	if err != nil {
		t.Errorf("OSVerify should succeed: %v", err)
	}
	if result != expected {
		t.Errorf("Expected '%s', got '%s'", expected, result)
	}
...
func TestOSVerifyFail(t *testing.T) {
	req := `{"image":"SONiC.master.0"}`
