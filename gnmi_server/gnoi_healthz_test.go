...
var testHealthzCases = []struct {
	...
	},
	{
		...
		f: func(ctx context.Context, t *testing.T, sc healthz.HealthzClient) {
			...
			dummy_containerfile := "/mnt/host/fake-collect-success"
			dummyData := []byte("dummy log data")
			_ = os.WriteFile(dummy_hostfile, dummyData, 0644)
			defer os.Remove(dummy_hostfile)

			_ = os.MkdirAll(filepath.Dir(dummy_containerfile), 0755)
			_ = os.WriteFile(dummy_containerfile, dummyData, 0644)
