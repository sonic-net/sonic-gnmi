...
type MockOSCfg struct {
	ProcessTrfReadyFunc func(string) (string, error)
	ProcessTrfEndFunc   func(string) (string, error)
}

func (m *MockOSCfg) ProcessTrfReady(req string) (string, error) {
	return m.ProcessTrfReadyFunc(req)
}
func (m *MockOSCfg) ProcessTrfEnd(req string) (string, error) {
	return m.ProcessTrfEndFunc(req)
}
...
var testOSCases = []struct {
	...
	},
	{
		...
		f: func(ctx context.Context, t *testing.T, sc ospb.OSClient, s *OSServer) {
			...
			s.config.OSCfg.ProcessTrfReady = func(_ string) (string, error) {
				return "", fmt.Errorf("OS Install not supported")
			}
...
func TestHandleErrorResponse(t *testing.T) {
	errResp := handleErrorResponse("something went wrong: %d", 42)
...
func TestHandleErrorResponse_MarshalError(t *testing.T) {
	...
	resp := handleErrorResponse("test marshal fail: %v", req)
