...
func isDebugData(p *types.Path) bool {
	...
	//	return false
	//}
	if (elems[2].GetName() != ddLogLvlAlert+ddLogLvlSuf && elems[2].GetName() != ddLogLvlCritical+ddLogLvlSuf && elems[2].GetName() != ddLogLvlAll+ddLogLvlSuf) || len(elems[2].GetKey()) > 0 {
		return false
	}
	return true
}
...
func getDebugData(p *types.Path) (*healthz.GetResponse, error) {
	...
	if len(elems) == 3 {
		...
		ll = strings.TrimSuffix(elems[2].GetName(), ddLogLvlSuf)
	}
	...
	/*
		...
		containerPath := filepath.Join("/tmp/dump", fileName)
		log.Infof("Reading artifact from: %s", containerPath)
	...
	*/
	...
	log.V(1).Infof("getDebugData(): artifact file read successfully, size=%d", len(fi))
