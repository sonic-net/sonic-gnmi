...
func NewGNSICredentialzServer(srv *Server) *GNSICredentialzServer {
	stateDbClient, err := common_utils.GetRedisDBClient()
	if err != nil {
...
// syncMetadataToDB performs the initial write to Redis using the persistent client
func (srv *GNSICredentialzServer) syncMetadataToDB() {
	if srv.stateDbClient == nil {
		log.V(0).Info("Cannot sync metadata: stateDbClient is nil")
		return
	}

	// SSH Host Sync
	srv.writeSshHostCredentialsMetadataToDB(sshCaKeysVersionFld, srv.sshCredMetadata.Host.CaKeysVersion)
	srv.writeSshHostCredentialsMetadataToDB(sshCaKeysCreatedOnFld, srv.sshCredMetadata.Host.CaKeysCreatedOn)

	// SSH Accounts Sync
	for account, meta := range srv.sshCredMetadata.Accounts {
		srv.writeSshAccountCredentialsMetadataToDB(account, sshKeysVersionFld, meta.KeysVersion)
		srv.writeSshAccountCredentialsMetadataToDB(account, sshKeysCreatedOnFld, meta.KeysCreatedOn)
		srv.writeSshAccountCredentialsMetadataToDB(account, sshPrincipalsVersionFld, meta.UsersVersion)
		srv.writeSshAccountCredentialsMetadataToDB(account, sshPrincipalsCreatedOnFld, meta.UsersCreatedOn)
	}

	// Console Accounts Sync
	for account, meta := range srv.consoleCredMetadata.Accounts {
		srv.writeConsoleAccountCredentialsMetadataToDB(account, consolePasswordVersionFld, meta.PasswordVersion)
		srv.writeConsoleAccountCredentialsMetadataToDB(account, consolePasswordCreatedOnFld, meta.PasswordCreatedOn)
	}
}

// Close cleans up the GNSICredentialzServer such as closing initialized clients.
func (srv *GNSICredentialzServer) Close() {
	if srv.stateDbClient != nil {
		if err := srv.stateDbClient.Close(); err != nil {
			log.Errorf("Error closing STATE_DB client: %v", err)
		}
	}
}
...
func (srv *GNSICredentialzServer) checkpointDelete(source cpType) {
	...
	if source == sshCP {
		if err := sc.SSHCheckpoint(ssc.CredzCPDelete); err != nil {
			log.V(1).Infof("Could not delete checkpoint: %v", err)
		}
	}
	if source == consoleCP {
...
func (srv *GNSICredentialzServer) updateGlomeState(ctx context.Context, newGlomeConfigMetadata *GlomeConfigMetadata) error {
	...
	if currentGlomeConfigMetadata, err := srv.readGlomeConfigMetadataFromStateDB(ctx); err != nil {
		log.V(1).Infof("failed to create STATE_DB checkpoint for GLOME: %v", err)
...
func (srv *GNSICredentialzServer) readGlomeConfigMetadataFromStateDB(ctx context.Context) (*GlomeConfigMetadata, error) {
	...
	glomeKey := common_utils.GetKey([]string{credentialsTbl, glomeConfigRedisKey})
