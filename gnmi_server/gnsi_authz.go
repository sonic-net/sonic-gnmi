...
func NewGNSIAuthzServer(srv *Server) *GNSIAuthzServer {
	...
	log.V(1).Info("gnsi: loading authz metadata from %s", srv.config.AuthzMetaFile)
	log.V(1).Info("gnsi: loading authz policy from %s", srv.config.AuthzPolicyFile)
...
func (srv *GNSIAuthzServer) Rotate(stream authz.Authz_RotateServer) error {
	...
	// log.V(3).Info("gNSI authz Rotate RPC disabled since NSF is ongoing")
	...
	if !authzMu.TryLock() {
		log.V(3).Infof("[%v]gNSI: authz.Rotate already in use", session)
		return status.Errorf(codes.Aborted, "concurrent authz.Rotate RPCs are not allowed")
	}
	defer authzMu.Unlock()
	log.V(1).Infof("[%v]gNSI: Begin authz.Rotate", session)
	defer log.V(1).Infof("[%v]gNSI: End authz.Rotate", session)
	srv.checkpointAuthzFreshness()
	if err := srv.checkpointAuthzFile(); err != nil {
		log.V(3).Infof("Failure during Authz checkpoint: %v", err)
	}
	for {
		req, err := stream.Recv()
		if err == io.EOF {
			log.V(3).Infof("[%v]gNSI: Received unexpected EOF", session)
			...
			if err := copyFile(srv.config.AuthzPolicyFile+backupExt, srv.config.AuthzPolicyFile); err != nil {
				log.V(3).Infof("[%v]gnsi: failed to revert authz policy file (%v): %v", session, srv.config.AuthzPolicyFile, err)
			}
			srv.revertAuthzFileFreshness()
			return status.Errorf(codes.Aborted, "No Finalize message")
		}
		if err != nil {
			log.V(3).Infof("[%v]gnsi: while processing a rotate request got error: `%v`. Reverting to last good state.", session, err)
			// Connection closed without Finalize message. Revert all changes made until now.
			if err := copyFile(srv.config.AuthzPolicyFile+backupExt, srv.config.AuthzPolicyFile); err != nil {
				log.V(3).Infof("[%v]gnsi: failed to revert authz policy file (%v): %v", session, srv.config.AuthzPolicyFile, err)
			}
		...
		if endReq := req.GetFinalizeRotation(); endReq != nil {
			...
			log.V(1).Infof("[%v]gNSI: Received Finalize: %v", session, endReq)
...
func (srv *GNSIAuthzServer) processRotateRequest(req *authz.RotateAuthzRequest) (*authz.RotateAuthzResponse, error) {
	...
	log.V(1).Infof("received a gNSI.Authz UploadRequest request message")
	log.V(4).Infof("request message: %v", policyReq)
...
func (srv *GNSIAuthzServer) checkpointAuthzFile() error {
	log.V(1).Infof("Checkpoint authz file: %v", srv.config.AuthzPolicyFile)
...
func (srv *GNSIAuthzServer) saveAuthzFileFreshess(path string) error {
	log.V(1).Infof("save authz metadata to file: %v", path)
...
func (srv *GNSIAuthzServer) loadAuthzFreshness(path string) error {
	log.V(1).Infof("load authz metadata from file: %v", path)
