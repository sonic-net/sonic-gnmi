...
func TestCopyFile_Errors(t *testing.T) {
	...
	t.Run("RenameError", func(t *testing.T) {
		// Line 42-46: Attempt to rename to a directory that doesn't exist
		// or where the user lacks write permissions.
		invalidDst := "/proc/invalid-path/dest"

		err := copyFile(srcPath, invalidDst)
		if err == nil {
			t.Error("expected error during os.Rename, got nil")
		}
	})
	t.Run("RenameError_and_Cleanup", func(t *testing.T) {
		srcPath := filepath.Join(t.TempDir(), "src.txt")
		os.WriteFile(srcPath, []byte("data"), 0644)

		// To make Rename fail: use a destination that is an existing directory
		invalidDst := t.TempDir()

		err := copyFile(srcPath, invalidDst)
		if err == nil {
			t.Error("expected error during os.Rename to a directory")
		}
	})
	t.Run("CopyFailure", func(t *testing.T) {
		srcPath := filepath.Join(t.TempDir(), "src.txt")
		os.WriteFile(srcPath, []byte("test"), 0644)

		// Use a destination path where CreateTemp succeeds but writing fails
		// Often achieved by pointing to a directory with a strict quota or
		// a filesystem that goes Read-Only.
		err := copyFile(srcPath, "/dev/null/invalid")
		if err == nil {
