...
const (
	...
	expectedSshCreateCmd          = "ssh_mgmt" + string(ssc.CredzCPCreate)
	expectedSshDeleteCmd          = "ssh_mgmt" + string(ssc.CredzCPDelete)
	expectedSshRestoreCmd         = "ssh_mgmt" + string(ssc.CredzCPRestore)
	expectedSshSetCmd             = "ssh_mgmt.set"
	expectedConsoleCreateCmd      = "gnsi_console" + string(ssc.CredzCPCreate)
	expectedConsoleDeleteCmd      = "gnsi_console" + string(ssc.CredzCPDelete)
	expectedConsoleRestoreCmd     = "gnsi_console" + string(ssc.CredzCPRestore)
	expectedConsoleSetCmd         = "gnsi_console.set"
	expectedGlomePushConfigCmd    = "glome" + string(ssc.CredzGlomePushConfig) // Used for both create checkpoint and set new glome config.
	expectedGlomeConfigRestoreCmd = "glome" + string(ssc.CredzCPRestore)
...
func TestGnsiCredzSSHServer(t *testing.T) {
	...
	var patches *gomonkey.Patches // Declare patches variable
	// Initialize the mock and the patch
	dbusListener, done, patches = newMockSshDbusServer()
	defer patches.Reset()
	defer func() { done <- true }() // Signal the mock goroutine to exit
	...
	if err := s.gnsiCredentialz.saveCredentialsFreshness(s.config.SshCredMetaFile); err != nil {
		t.Fatal(err)
	}
}

func newMockSshDbusServer() (listenerChan chan []string, done chan bool, patches *gomonkey.Patches) {
	// dbusServerInputChan: This is the channel that the FakeClient will write to.
	dbusServerInputChan := make(chan []string, 10)
	// listenerChan: This is the channel that the test function (via dbusListen) will read from.
	listenerChan = make(chan []string, 10)
	done = make(chan bool, 1)

	// Initialize our FakeClient, directing its output to dbusServerInputChan.
	fakeService := &ssc.FakeClient{Command: dbusServerInputChan}

	// PATCH: Force ssc.NewDbusClient to return the FakeClient.
	...
	go func() {
		...
		for {
			...
			case <-done: // if cancel() execute
				log.V(2).Infoln("Shutting down the DBUS server.")
				return
			case msg := <-dbusServerInputChan:
				mu.Lock()
				log.V(2).Infof("DBUS service received %+v", msg)
				switch msg[0] {
				...
				default:
					log.V(2).Infof("mock ssh_mgmt service: unknown service: %+v", msg)
				}
				listenerChan <- msg
				mu.Unlock()
			}
	...
	return listenerChan, done, patches
}
...
func run(name string, dbus <-chan []string, t *testing.T, cmd []string, f func() error) {
	t.Run(name, func(t *testing.T) {
		...
		for i := 0; i < count; i++ {
			select {
			case resp := <-dbus:
				log.V(2).Infof("received on DBUS: %v\n", resp)
				for i := range cmd {
					if cmd[i] != resp[i] {
			...
			case err := <-finished:
				log.V(2).Infof("f() is done with err=%v\n", err)
				if err != nil {
					t.Error(err.Error())
				}
			case <-time.After(time.Second * 3):
	...
	t.Run(testName, func(t *testing.T) {
		...
		for _, expectedDbusMsg := range expectedDbusMsgs {
			...
			case resp := <-dbusListener:
				...
				// 3. Handle Set/Push commands (which have JSON payload)
				if len(resp) < 2 {
					t.Errorf("expected JSON payload for method %v, but got none", resp[0])
					continue
				}
		...
		select {
		case err := <-finished:
			log.V(2).Infof("clientAction() is done with err=%v\n", err)
			if err != nil {
				t.Error(err.Error())
			}
		case <-time.After(time.Second * 5):
		...
		log.V(2).Infoln("Finished:", testName)
	})
}
...
var consoleTests = []struct {
	...
	},
	{
		...
		f: func(t *testing.T, ctx context.Context, sc credz.CredentialzClient, ch <-chan []string) {
			...
			// Check for Canceled or DeadlineExceeded
			code := status.Code(err)
			if code != codes.Canceled && code != codes.DeadlineExceeded {
				t.Fatalf("Unexpected error code: %v; err: %v", code, err)
			}
			if resp != nil {
				t.Fatal("Received unexpected message after closing connection.")
			}
			select {
			case msg := <-ch:
				if msg[0] != expectedConsoleCreateCmd && msg[0] != expectedConsoleRestoreCmd {
					t.Errorf("Unexpected DBUS msg %v", msg)
				}
			default:
			}
		},
...
func TestGnsiCredzConsoleServer(t *testing.T) {
	...
	dbusListener, done, patches = newMockConsoleDbusServer()
	defer patches.Reset()
...
func TestGnsiCredzConsoleServerNoDBUS(t *testing.T) {
	...
	dbusListener, done, patches = newFailingConsoleDbusServer()
	defer patches.Reset()
	defer func() { done <- true }()
	...
	s.gnsiCredentialz.saveConsoleCredentialsFreshness(s.config.ConsoleCredMetaFile)
}

func newMockConsoleDbusServer() (listenerChan chan []string, done chan bool, patches *gomonkey.Patches) {
	dbusServerInputChan := make(chan []string, 10)
	listenerChan = make(chan []string, 10)
	done = make(chan bool, 1)
	...
	// PATCH: Force ssc.NewDbusClient to return the FakeClient.
	patches = gomonkey.ApplyFunc(ssc.NewDbusClient, func() (ssc.Service, error) {
		fmt.Printf("DEBUG: MOCK: ssc.NewDbusClient() called, returning FakeClient")
		return fakeService, nil
	})
...
func newFailingConsoleDbusServer() (listenerChan chan []string, done chan bool, patches *gomonkey.Patches) {
	dbusServerInputChan := make(chan []string, 10)
	listenerChan = make(chan []string, 10)
	done = make(chan bool, 1)
...
func TestGnsiCredzGlome(t *testing.T) {
	...
	// Initialize the mock and the patch
	dbusListener, done, patches = newMockGlomeDbusServer()
	defer patches.Reset()
	defer func() { done <- true }()
...
func newMockGlomeDbusServer() (listenerChan chan []string, shutdownChan chan bool, patches *gomonkey.Patches) {
	...
	return listenerChan, shutdownChan, patches
}
