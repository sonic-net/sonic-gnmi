...
func TestGnsiCredzSSHServer(t *testing.T) {
	...
	dbusListener, done, resetFunc = newMockSshDbusServer()
	defer resetFunc()
...
func newMockSshDbusServer() (chan []string, chan bool, func()) {
	...
	// 1. Save the original provider
	originalProvider := ssc.NewDbusClientProvider

	// 2. Overwrite the provider with our mock
	ssc.NewDbusClientProvider = func() (ssc.Service, error) {
		return fakeService, nil
	}

	// 3. Define the cleanup/reset function
	resetFunc := func() {
		ssc.NewDbusClientProvider = originalProvider
	}
	go func() {
		checkpoint := false
		var mu sync.Mutex
	...
	return listenerChan, done, resetFunc
}
...
func TestGnsiCredzConsoleServer(t *testing.T) {
	...
	dbusListener, done, resetFunc = newMockConsoleDbusServer()
	defer resetFunc()
...
func TestGnsiCredzConsoleServerNoDBUS(t *testing.T) {
	...
	dbusListener, done, resetFunc = newFailingConsoleDbusServer()
	defer resetFunc()
	defer func() { done <- true }()
...
func newMockConsoleDbusServer() (chan []string, chan bool, func()) {
	dbusServerInputChan := make(chan []string, 10)
	listenerChan := make(chan []string, 10)
	done := make(chan bool, 1)
...
func newFailingConsoleDbusServer() (chan []string, chan bool, func()) {
	dbusServerInputChan := make(chan []string, 10)
	listenerChan := make(chan []string, 10)
	done := make(chan bool, 1)
...
func TestGnsiCredzGlome(t *testing.T) {
	...
	dbusListener, done, resetFunc = newMockGlomeDbusServer()
	defer resetFunc()
	defer func() { done <- true }()
...
func newMockGlomeDbusServer() (chan []string, chan bool, func()) {
	...
	return listenerChan, shutdownChan, resetFunc
}
