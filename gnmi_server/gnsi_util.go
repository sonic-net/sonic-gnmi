...
func getRedisDBClient(dbName string) (*redis.Client, error) {
	rclient := newClientFunc(dbName)
	if rclient == nil {
		return nil, fmt.Errorf("failed to initialize redis client: connection is nil")
	}
...
func closeRedis(sc *redis.Client) error {
	return sc.Close()
}

func newRedisClient(dbName string) *redis.Client {
