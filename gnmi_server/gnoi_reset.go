...
func (srv *Server) Start(ctx context.Context, req *factory_reset.StartRequest) (*factory_reset.StartResponse, error) {
	...
	if err != nil {
		rerr := factory_reset.ResetError{Detail: err.Error()}
		resp.Response = &factory_reset.StartResponse_ResetError{ResetError: &rerr}
		return resp, nil
	} else {
		resp.Response = &factory_reset.StartResponse_ResetSuccess{}
		return resp, nil
	}
}
