...
func runTestSubscribe(t *testing.T, namespace string) {
	...
	var tmpRate interface{}
	json.Unmarshal(ratesEthernetWildcardByte, &tmpRate)
	ratesEthernet68JsonUpdate := tmpRate.(map[string]interface{})
	ratesEthernet68JsonUpdate["test_field"] = "test_value"
	var ratesEthernetWildcardJson interface{}
	json.Unmarshal(ratesEthernetWildcardByte, &ratesEthernetWildcardJson)
	// Will have "test_field" : "test_value" in Ethernet68,
	ratesEtherneWildcardJsonUpdate := map[string]interface{}{"Ethernet68/1": ratesEthernet68JsonUpdate}
	...
	tests := []TestExec{
		...
		{
			desc: "stream query for key Ethernet* with new test_field field on Ethernet68",
			...
			wantNoti: []client.Notification{
				...
				client.Sync{},
				client.Update{Path: []string{"COUNTERS_DB", "RATES", "Ethernet*"}, TS: time.Unix(0, 200), Val: ratesEtherneWildcardJsonUpdate},
			},
		},
		{
		...
		},
		{
			...
			generateIntervals: true,
			updates: []tablePathValue{
				createRatesTableSetUpdate("oid:0x1000000000056", "test_field", "test_value"),
			},
			wantNoti: []client.Notification{
				client.Connected{},
				client.Update{Path: []string{"COUNTERS_DB", "RATES", "Ethernet*"}, TS: time.Unix(0, 200), Val: ratesEthernetWildcardJson},
				client.Sync{},
				client.Update{Path: []string{"COUNTERS_DB", "RATES", "Ethernet*"}, TS: time.Unix(0, 200), Val: mergeStrMaps(ratesEthernetWildcardJson, ratesEtherneWildcardJsonUpdate)},
			},
		},
		{
		...
		{
			desc:              "sample stream query for  RATES/Ethernet89/FEC_PRE_BER",
			q:                 createCountersDbQuerySampleMode(t, 0, false, "COUNTERS", "Ethernet89"),
			generateIntervals: true,
			updates: []tablePathValue{
				createRatesTableSetUpdate("oid:0x1000000000056", "FEC_PRE_BER", "3"),
			},
			wantNoti: []client.Notification{
				client.Connected{},
				client.Update{Path: []string{"COUNTERS_DB", "RATES", "Ethernet89"}, TS: time.Unix(0, 200), Val: countersEthernet68Json},
				client.Sync{},
				client.Update{Path: []string{"COUNTERS_DB", "RATES", "Ethernet89"}, TS: time.Unix(0, 200), Val: countersEthernet68JsonUpdate},
			},
		},
		{
