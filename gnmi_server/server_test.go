...
func TestGnmiGetPortChannelLagType(t *testing.T) {
	...
	t.Cleanup(func() {
		...
		configClient.Del(cleanupCtx, "PORTCHANNEL|PortChannel01", "PORTCHANNEL|PortChannel02")
	})
...
func TestGnmiGetTranslibXfmrPortChannel(t *testing.T) {
	...
	t.Cleanup(func() {
		cleanupCtx := context.Background()
		configClient.Del(cleanupCtx, "PORT|Ethernet0", "PORTCHANNEL|PortChannel111", "PORTCHANNEL_MEMBER|PortChannel111|Ethernet0")
		applClient.Del(cleanupCtx, "LAG_TABLE:PortChannel111", "LAG_MEMBER_TABLE:PortChannel111:Ethernet0")
		stateClient.Del(cleanupCtx, "LAG_TABLE|PortChannel111", "LAG_MEMBER_TABLE|PortChannel111|Ethernet0")
	})
	...
	// 6. Test execution with gotVal logger
	for _, td := range tds {
		t.Run(td.desc, func(t *testing.T) {
			ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
			defer cancel()
...
func TestGnmiLagTypeTransformer(t *testing.T) {
	...
	t.Cleanup(func() {
		cleanupCtx := context.Background()
		configClient.Del(cleanupCtx, "PORTCHANNEL|"+lagName)
	})
