...
func createMTLSServer(t *testing.T, caCert string) *Server {
	t.Helper()
	cfg := &Config{CaCertLnk: CACert, CaCertFile: caCert, SrvCertFile: SCertV1, SrvKeyFile: SKeyV1, CertzMetaFile: certzTestMetaFile, CertCRLConfig: crlConfigTestPath, GetOptions: SrvAdvConfig, SrvCertLnk: srvTestCertLink, SrvKeyLnk: srvTestKeyLink, Port: 8081}
	if err := resetSrvCertKeyToV1(cfg); err != nil {
		t.Errorf("3:24 Couldn't reset server certificate/key: %v", err)
	}
