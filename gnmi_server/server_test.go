...
func TestGnmiGetSoftwareComponents(t *testing.T) {
	...
	t.Cleanup(func() {
		cleanupCtx := context.Background()
		if err := stateClient.Del(cleanupCtx, "DEVICE_METADATA|localhost", "software_version").Err(); err != nil {
			t.Logf("Failed to HDel swver field: %v", err)
		}
		if err := stateClient.Del(cleanupCtx, "COMPONENT|os0").Err(); err != nil {
			t.Logf("Failed to HDel COMPONENT|os0 field: %v", err)
		}
		if err := stateClient.Del(cleanupCtx, "COMPONENT|boot_loader0").Err(); err != nil {
			t.Logf("Failed to HDel COMPONENT|bootloader0 field: %v", err)
		}
		if err := stateClient.Del(cleanupCtx, "COMPONENT|network_stack0").Err(); err != nil {
			t.Logf("Failed to HDel COMPONENT|nwstack0 field: %v", err)
		}
		stateClient.Close()
	})
