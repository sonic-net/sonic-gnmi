...
func TestDbTableKeySubscribe_EnqueFatalMsg(t *testing.T) {
	...
	// Use a dummy Redis client (will fail if Redis isn't running)
	dummyRedis := redis.NewClient(&redis.Options{
		Addr: "localhost:6379", // or any unreachable address
	})
...
type MockRedisClient struct {
	PSubscribeFunc func(pat string) *redis.PubSub
}

func (m *MockRedisClient) PSubscribe(pat string) *redis.PubSub {
	if m.PSubscribeFunc != nil {
		return m.PSubscribeFunc(pat)
	}
	return nil
}
