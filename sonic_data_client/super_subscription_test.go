...
func TestFindSuperSubscription_NilRequestInMap(t *testing.T) {
	// 1. Setup the input subscription we are looking for
	targetSub := &pb.SubscriptionList{
		Mode: pb.SubscriptionList_STREAM,
	}

	// 2. Initialize or mock superSubs to contain a nil request
	superSubs.subs = make(map[*superSubscription]bool)

	// Entry A: The nil request case
	badEntry := &superSubscription{request: nil}
	superSubs.subs[badEntry] = true

	// Entry B: A valid request that doesn't match
	wrongEntry := &superSubscription{
		request: &pb.SubscriptionList{Mode: pb.SubscriptionList_ONCE},
	}
	superSubs.subs[wrongEntry] = true

	// 3. Execution
	result := findSuperSubscription(targetSub)

	// 4. Validation
	if result != nil {
		t.Errorf("Expected nil return when no matches exist, even with nil requests in map")
	}
}
func TestSuperSubscription_String(t *testing.T) {
	// 1. Setup mock data
	ss := &superSubscription{
		clients: make(map[*TranslClient]struct{}),
		request: &pb.SubscriptionList{
			Mode: pb.SubscriptionList_STREAM,
		},
	}

	// Set some values for the atomic counters
	ss.sharedUpdates.Store(10)
	ss.exclusiveUpdates.Store(5)
