...
func TestDownloadFile_Success(t *testing.T) {
	...
	tempDir := t.TempDir()
	outputPath := filepath.Join(tempDir, "test-file.bin")

	// Perform download
	ctx := context.Background()
	session, result, err := DownloadFile(ctx, server.URL, outputPath)
...
func TestDownloadFile_AutoOutputPath(t *testing.T) {
	// Create a test HTTP server
	testContent := "test file content"
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
	...
	downloadURL := server.URL + "/test-file.bin"

	// Perform download without output path
	ctx := context.Background()
	session, result, err := DownloadFile(ctx, downloadURL, "")
...
func TestDownloadFile_NetworkError(t *testing.T) {
	...
	invalidURL := "http://192.0.2.1:12345/test-file.bin" // RFC3330 test address
	...
	session, result, err := DownloadFileWithConfig(ctx, invalidURL, outputPath, config)
...
func TestDownloadFile_FileSystemError(t *testing.T) {
	// Create a test HTTP server
	...
	invalidPath := "/proc/invalid/test-file.bin" // /proc is read-only

	ctx := context.Background()
	session, result, err := DownloadFile(ctx, server.URL, invalidPath)
...
func TestGetOutputPathFromURL(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			url:      "http://example.com/test-file.bin",
			expected: "test-file.bin",
		},
		{
			name:     "path with directories",
			url:      "http://example.com/path/to/test-file.bin",
			expected: "test-file.bin",
		},
		{
			name:     "filename with query params",
			url:      "http://example.com/test-file.bin?version=1.2.3",
			expected: "test-file.bin",
		},
		{
			name:        "no filename",
...
func TestCreateSuccessResult(t *testing.T) {
	...
	result := createSuccessResult(
		"http://example.com/test-file.bin",
		"/tmp/test-file.bin",
		startTime,
		3,
		"direct",
	...
	assert.Equal(t, "http://example.com/test-file.bin", result.URL)
	assert.Equal(t, "/tmp/test-file.bin", result.FilePath)
...
// TestDownloadFile_Integration tests the full download process with a real HTTP server.
func TestDownloadFile_Integration(t *testing.T) {
	...
	t.Run("successful download", func(t *testing.T) {
		...
		ctx := context.Background()
		session, result, err := DownloadFile(ctx, server.URL+"/success", outputPath)

		require.NoError(t, err)
		require.NotNil(t, session)
		require.NotNil(t, result)
		assert.Equal(t, int64(9), result.FileSize)

		content, err := os.ReadFile(outputPath)
		require.NoError(t, err)
		assert.Equal(t, "test file", string(content))
	})
	...
	t.Run("timeout handling", func(t *testing.T) {
		...
		session, result, err := DownloadFileWithConfig(ctx, server.URL+"/slow", outputPath, config)
