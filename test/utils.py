...
def gnoi_client_cmd_common():
    """
    Returns the gnoi_client command with default target
    """
    path = os.getcwd()
...
def gnoi_reboot(method, delay, message):
    cmd = gnoi_client_cmd_common()
    cmd += "-rpc Reboot "
    cmd += '-jsonin "{\\"method\\":%d, \\"delay\\":%d, \\"message\\":\\"%s\\"}"' % (
        method,
        delay,
        message,
    )
...
def gnoi_authenticate(username, password):
    ...
    cmd += '-jsonin "{\\"Username\\":\\"%s\\", \\"Password\\":\\"%s\\"}"' % (
        username,
        password,
    )
    ret, msg = run_cmd(cmd)
