...
from utils import (
    gnmi_subscribe_poll,
    gnmi_subscribe_stream_sample,
    gnmi_subscribe_stream_onchange,
)
...
test_json_checkpoint = {
    ...
    "DASH_VNET": {
        "vnet_3721": {"address_spaces": ["10.250.0.0", "192.168.3.0", "139.66.72.9"]}
    },
}
...
test_data_checkpoint = [
    ...
    [
        ...
        },
        {
            ...
            "value": {"address_spaces": ["10.250.0.0", "192.168.3.0", "139.66.72.9"]},
        },
    ],
    [
        {"path": "/sonic-db:CONFIG_DB/localhost/DASH_QOS/qos_01/flows", "value": "300"},
        {"path": "/sonic-db:CONFIG_DB/localhost/DASH_QOS/qos_02/bw", "value": "6000"},
        {
...
class TestGNMIConfigDb:
    ...
    def test_gnmi_incremental_update(self, test_data):
        ...
        for item in test_data:
            ...
            for patch_data in patch_json:
                ...
                if (
                    test_path == "/sonic-db:CONFIG_DB/localhost" + patch_data["path"]
                    and test_value == patch_data["value"]
                ):
                    break
            else:
        ...
        assert ret == 0, "Fail to read counter"
        assert (
            new_create_checkpoint_cnt == old_create_checkpoint_cnt + 1
        ), "DBUS API is not invoked"
        ret, new_delete_checkpoint_cnt = gnmi_dump("DBUS delete checkpoint")
        assert ret == 0, "Fail to read counter"
        assert (
            new_delete_checkpoint_cnt == old_delete_checkpoint_cnt + 1
        ), "DBUS API is not invoked"
    ...
    def test_gnmi_get_checkpoint_negative_01(self):
        ...
        get_list = [
            "/sonic-db:CONFIG_DB/localhost/DASH_VNET/vnet_3721/address_spaces/0/abc"
        ]
    ...
    def test_gnmi_get_checkpoint_negative_02(self):
        ...
        get_list = [
            "/sonic-db:CONFIG_DB/localhost/DASH_VNET/vnet_3721/address_spaces/abc"
        ]
    ...
    def test_gnmi_get_checkpoint_negative_03(self):
        ...
        get_list = [
            "/sonic-db:CONFIG_DB/localhost/DASH_VNET/vnet_3721/address_spaces/1000"
        ]
    ...
    def test_gnmi_stream_onchange_01(self):
        ...
        for i in range(cnt):
            time.sleep(0.5)
            cmd = r'redis-cli -n 4 hset "DEVICE_METADATA|localhost" bgp_asn ' + str(
                i + 1000
            )
            run_cmd(cmd)
